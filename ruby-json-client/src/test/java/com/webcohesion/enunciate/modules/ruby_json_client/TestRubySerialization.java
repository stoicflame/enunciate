...
import com.webcohesion.enunciate.examples.ruby_json_client.schema.structures.House;
import com.webcohesion.enunciate.examples.ruby_json_client.schema.vehicles.Bus;
 ...
 * @author Ryan Heaton
 */
public class TestRubySerialization extends TestCase {

  private boolean skipRubyTests;
  ...
  protected void setUp() throws Exception {
    ...
    String skipRubyTests = System.getProperty("skip.ruby.tests");
    this.skipRubyTests = "true".equalsIgnoreCase(skipRubyTests);

    if (!this.skipRubyTests) {
      this.rubyExe = System.getProperty("ruby.execuable");
      if (this.rubyExe == null) {
        throw new IllegalStateException("The ruby executable must be supplied via property 'ruby.executable'.");
      }

      String exe = System.getProperty("processjson.rb");
      if (exe == null) {
...
//    house.setConstructedDate(new DateTime(date, DateTimeZone.UTC));
    ...
    assertNull("constructed date should be null (ruby value type should not be serialized)", house.getConstructedDate());
