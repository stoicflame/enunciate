...
import com.webcohesion.enunciate.examples.javascript_client.schema.animals.Cat;
import com.webcohesion.enunciate.examples.javascript_client.schema.draw.Canvas;
import com.webcohesion.enunciate.examples.javascript_client.schema.structures.House;
import com.webcohesion.enunciate.examples.javascript_client.schema.vehicles.Bus;
...
/**
 * Makes sure JavaScript serialization is working correctly.
 *
 * @author Ryan Heaton
 */
public class TestJavaScriptSerialization extends TestCase {

  private boolean skipJSTests;
  ...
  protected void setUp() throws Exception {
    ...
    String skipJSTests = System.getProperty("skip.js.tests");
    this.skipJSTests = "true".equalsIgnoreCase(skipJSTests);

    if (!this.skipJSTests) {
      this.nodeExe = System.getProperty("node.executable");
      if (this.nodeExe == null) {
        throw new IllegalStateException("The node executable must be supplied via property 'node.executable'.");
      }

      String exe = System.getProperty("processjson.js");
      if (exe == null) {
        throw new IllegalStateException("The path to the processjson script must be supplied via property 'processjson.js'.");
      }
  ...
  public void testSerializeDeserialize() throws Exception {
    if (this.skipJSTests) {
      System.out.println("JavaScript tests have been disabled.");
      return;
    }
...
//    house.setConstructedDate(new DateTime(date, DateTimeZone.UTC));
    ...
    assertNull("constructed date should be null (js value type should not be serialized)", house.getConstructedDate());
...
//    assertEquals(3, attachmentCount);
  ...
  protected <T> T processThroughJson(T object) throws Exception {
    ...
    fos.close();
    Process process = new ProcessBuilder(this.nodeExe, this.exe.getAbsolutePath(), packageToModule(object.getClass().getName()), in.getAbsolutePath(), out.getAbsolutePath())
    ...
    assertEquals("node process json failed.", 0, exitStatus);
