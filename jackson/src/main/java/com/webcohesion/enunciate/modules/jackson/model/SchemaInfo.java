...
public class SchemaInfo {
  ...
  public SchemaInfo(EnunciateJacksonContext context) {
   ...
   *
   * @param jsonType The xml type.
   * @param referencedNamespaces The set of referenced namespaces.
   */
  private void addReferencedNamespaces(JsonType jsonType, LinkedList<String> stack, Set<String> referencedNamespaces) {
    if (!jsonType.isAnonymous()) {
      referencedNamespaces.add(jsonType.getNamespace());
    }
    else if (jsonType instanceof JsonMapType) {
      referencedNamespaces.add(((JsonMapType) jsonType).getKeyType().getNamespace());
      referencedNamespaces.add(((JsonMapType) jsonType).getValueType().getNamespace());
    }

    if (jsonType instanceof JsonClassType) {
      addReferencedNamespaces(((JsonClassType) jsonType).getTypeDefinition(), stack, referencedNamespaces);
    }
  }
